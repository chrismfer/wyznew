...
async function checkout() {
    ...
    const modalContent = pixModal ? pixModal.querySelector('.modal-content') : null;

    if (!pixModal || !modalContent) {
        console.error("Elementos do modal PIX não encontrados.");
        mostrarNotificacao("Erro ao iniciar pagamento.", "erro");
        return;
    }
    ...
    const itensParaPagamento = JSON.stringify(cart.map(item => ({ 
        code: item.code, 
        nome: item.nome, 
        preco: item.preco 
    })));
    ...
    try {
        const res = await fetchAPI(`${scriptURL}?action=criarPagamentoPix&email=${encodeURIComponent(currentUser)}&nome=${encodeURIComponent(currentUserName)}&valor=${total}&itens=${encodeURIComponent(itensParaPagamento)}`);
...
async function checkoutDiretoVIP(productCode, buttonId) {
    ...
    const itensParaPagamento = JSON.stringify([{
        code: produtoVIP.code,
        nome: produtoVIP.nome,
        preco: preco
    }]);

    try {
        const res = await fetchAPI(`${scriptURL}?action=criarPagamentoPix&email=${encodeURIComponent(currentUser)}&nome=${encodeURIComponent(currentUserName)}&valor=${preco}&itens=${encodeURIComponent(itensParaPagamento)}`);
...
function injectCartProfessionalStyles() {
    ...
    style.textContent = `
        ...
        .checkout-btn-fixed .spinner { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); opacity: 0; visibility: hidden; transition: opacity 0.2s, visibility 0.2s; }
        ...
        .pix-body { padding: 20px; text-align: center; display: flex; flex-direction: column; gap: 12px; }
        .pix-qrcode-container { padding: 8px; background: rgba(255,255,255,0.95); border-radius: var(--border-radius-md); margin: 0 auto; display: inline-block; animation: qr-glow 2.5s infinite ease-in-out; }
        .qr-code-image { width: 180px; height: 180px; display: block; border-radius: var(--border-radius-sm); }
        .pix-scan-text { font-size: 0.8rem; color: var(--text-light-muted); margin: 0; }
        .pix-separator { display: none; }
        .pix-copy-text { font-size: 0.8rem; color: var(--text-light-muted); margin: 0; }
        .pix-key-container { display: flex; margin: 0; }
