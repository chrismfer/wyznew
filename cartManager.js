...
function checkout() {
    if (cart.length === 0) {
        return void mostrarNotificacao("Seu carrinho está vazio", "erro");
    }
    showLoading("checkout-btn");
    const total = calcularTotal();
    const itensCarrinho = cart.map(item => item.nome).join(", ");
    fetch(`${scriptURL}?action=criarPagamentoPix&email=${encodeURIComponent(currentUser)}&nome=${encodeURIComponent(currentUserName)}&valor=${total}&produto=${encodeURIComponent(itensCarrinho)}`)
    .then(res => res.json())
    .then(res => {
        ...
        if (res.sucesso) {
            ...
            if (!pixModal || !qrCodeImg || !pixKeyInput) {
                return console.error("Elementos do modal PIX não encontrados."), void mostrarNotificacao("Erro ao exibir modal de pagamento.", "erro");
            }
...
function checkoutDiretoVIP(productCode, buttonId) {
    if (!buttonId) {
    ...
    if (!produtoVIP) {
        return void mostrarNotificacao("Produto de assinatura não encontrado. Contate o suporte.", "erro");
    }
    
    const preco = parseFloat(produtoVIP.valorAvulso) || 0;
    if (preco <= 0) {
        return void mostrarNotificacao("Preço inválido para o produto VIP. Contate o suporte.", "erro");
    }
    ...
    fetch(`${scriptURL}?action=criarPagamentoPix&email=${encodeURIComponent(currentUser)}&nome=${encodeURIComponent(currentUserName)}&valor=${preco}&produto=${encodeURIComponent(nomeProduto)}`)
    .then(res => res.json())
    .then(res => {
        if (res.sucesso) {
            sessionStorage.setItem("pagamentoAtual", res.idPagamento);
...
function iniciarVerificacaoStatus(idPagamento, carrinhoAtual) {
    ...
    verificacaoStatusInterval = setInterval(() => {
        fetch(`${scriptURL}?action=verificarStatusPagamentoManual&idPagamento=${idPagamento}`)
        .then(res => res.json())
        .then(res => {
...
function processarConfirmacaoPagamento(idPagamento, carrinhoConfirmacao = null) {
...
function injectCartProfessionalStyles() {
    ...
    style.textContent = `
        ...
        /* AJUSTE SPINNER: Adicionada animação dedicada para o spinner do PIX */
        ...
        #status-verificacao .spinner {
            ...
            border-radius: 50%;
            /* AJUSTE SPINNER: Aplicando a animação de rotação simples */
