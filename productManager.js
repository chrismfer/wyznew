    ...
    if (isPedidos) {
        container.querySelectorAll('.download-btn').forEach(button => {
            ...
            newButton.addEventListener('click', function(e) {
                ...
                fetch(`${scriptURL}?action=getDownloadLink&email=${encodeURIComponent(currentUser)}&code=${code}`)
                    .then(res => res.json())
                    .then(res => {
...
function debugProdutos(code = null) {
    console.group('🔍 Debug do Sistema de Produtos');
    console.log('Total de produtos carregados:', produtos.length);
    console.log('Produtos no window:', window.produtos ? window.produtos.length : 'undefined');
    console.log('Função openProductDetails disponível:', typeof window.openProductDetails);
    console.log('Função abrirDetalhesProduto disponível:', typeof window.abrirDetalhesProduto);
    
    if (code) {
        console.log(`Buscando produto ${code}:`);
        const produto = produtos.find(p => p.code === code);
        console.log('Encontrado no array local:', produto ? 'SIM' : 'NÃO');
        
        const produtoWindow = window.produtos ? window.produtos.find(p => p.code === code) : null;
        console.log('Encontrado no window.produtos:', produtoWindow ? 'SIM' : 'NÃO');
        
        const produtoDOM = document.querySelector(`#product-grid .product-card[data-id="${code}"]`);
        console.log('Encontrado no DOM:', produtoDOM ? 'SIM' : 'NÃO');
    }
    console.groupEnd();
}
...
function _processarDadosDeProdutos(dadosAPI) {
    ...
    produtos.forEach(produto => {
        if (produto.nome.toLowerCase().includes('windows 10')) {
            produto.categoria = 'win10';
        } else if (produto.nome.toLowerCase().includes('windows 11')) {
            produto.categoria = 'win11';
        } else {
            produto.categoria = 'outros';
        }
    });

    const todasImagens = produtos.flatMap(p => [p.imagem, ...(p.imagensAdicionais || [])]).filter(Boolean);
    if (typeof preCarregarImagens === 'function') {
        preCarregarImagens(todasImagens);
    }
    
    carregarProdutos();
}

function carregarProdutosDaPlanilha() {
    ...
    fetch(`${scriptURL}?action=getProdutos&email=${encodeURIComponent(currentUser)}`)
        .then(res => {
            if (!res.ok) {
        ...
        .catch(err => {
            ...
            productGrid.innerHTML = `
                <div class="placeholder-message erro-placeholder">
                    Erro ao carregar produtos. Tente novamente mais tarde.<br>
                    <button onclick="carregarProdutosDaPlanilha()" class="retry-button">
                        <i class="fas fa-sync-alt"></i> Tentar novamente
                    </button>
                </div>`;
            
            if (produtos.length > 0) {
                console.log('Usando produtos em cache após erro de carregamento');
                carregarProdutos();
                registrarFuncoesGlobais();
            }
        });
...
function getPrecoDisplay(produto, isGratuito = false) {
    ...
    if (isGratuito) {
        ...
        htmlPreco = `
            ${precoAntigoHTML}
            <span class="product-price-highlight">Grátis</span>
        `;
    } else if (isVIP && valorAssinante > 0) {
        htmlPreco = `
            <span class="product-price-old">${formatarMoeda(valorAvulso)}</span>
            <span class="product-price-highlight vip-price">${formatarMoeda(valorAssinante)}</span>
        `;
    } else if (temPromocao) {
        htmlPreco = `
            <span class="product-price-old">${formatarMoeda(valorAvulso)}</span>
            <span class="product-price-highlight">${formatarMoeda(valorPromocao)}</span>
        `;
    } else {
