    ...
    fetch(`${scriptURL}?action=verificarIdentificador&identificador=${encodeURIComponent(identificadorProcessado)}`)
        ...
        .then(res => {
            ...
            if (res.sucesso) {
                ...
                mostrarNotificacao('Cadastro Localizado!', 'sucesso');
    ...
    fetch(scriptURL, { method: 'POST', body: formData })
        ...
        .then(res => {
            ...
            if (res.sucesso) {
                ...
                mostrarNotificacao('Sucesso!', 'sucesso');
                setTimeout(() => {
    ...
    fetch(scriptURL, { method: 'POST', body: formData })
        ...
        .then(res => {
            ...
            if (!res.sucesso && res.cpfExistente) {
                mostrarNotificacao('E-mail ou CPF já cadastrado!', 'erro');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (params.get('action') === 'cadastro') {
        toggleCadastro(true);
    } else if (params.get('action') === 'recuperar') {
        toggleEsqueceuSenha(true);
    }
