...
function openProductDetails(code, source = 'loja') {
    ...
    if (!produto) {
        ...
        console.log('Debug info:', {
            windowProdutos: window.produtos ? window.produtos.length : 'undefined',
            localProdutos: typeof produtos !== 'undefined' ? produtos.length : 'undefined',
            domElements: document.querySelectorAll(`[data-id="${code}"]`).length
        });
...
function carregarImagensProduto(code, imagemPrincipal, callback) {
    ...
    fetch(`${scriptURL}?action=getProdutoImagens&code=${encodeURIComponent(code)}`)
        .then(res => res.json())
        .then(res => {
...
function carregarDetalheProduto(code, callback) {
    if (detalhesCache[code] && detalhesCache[code].descricao) {
        callback(detalhesCache[code].descricao);
        return;
    }
    
    fetch(`${scriptURL}?action=getDescricaoProduto&code=${encodeURIComponent(code)}`)
        .then(res => res.json())
        .then(res => {
...
function carregarTodosOsProdutos(callback) {
    ...
    fetch(`${scriptURL}?action=getTodosProdutos&codes=${encodeURIComponent(codigosString)}`)
        .then(res => res.json())
        .then(res => {
            if (res.sucesso) {
